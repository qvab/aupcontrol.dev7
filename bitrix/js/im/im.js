	...
	if (historyElements[userId] && (!userIsChat && this.users[userId] || userIsChat && this.chat[chatId]))
	{
		...
		for (var i = 0; i < arHistorySort.length; i++)
		{
			...
			var findText = function(input, text)
			{
				...
				{
					if (input.toLowerCase().includes(text.toLowerCase()))
					{
						isTextInParams = true;
					}
				}
			...
			var isTextInAuthorName = false;
			if (this.message[historyElements[userId][i]].senderId > 0)
			{
				if (
					this.users[this.message[historyElements[userId][i]].senderId]
					&& this.users[this.message[historyElements[userId][i]].senderId].name.toLowerCase().indexOf((this.historySearch+'').toLowerCase()) >= 0)
				{
					isTextInAuthorName = true;
				}
			}

			var isTextInFileNames = false;
			if (this.message[historyElements[userId][i]].params && this.message[historyElements[userId][i]].params.FILE_ID)
			{
				...
				{
					if (!this.disk.files[messageChatId] || this.disk.files[messageChatId][file])
					{
						return true;
					}

					if (this.disk.files[messageChatId][file].name.toLowerCase().indexOf((this.historySearch+'').toLowerCase()) >= 0)
					{
						isTextInFileNames = true;
					}
				}, this);
			}
			...
			if (
				activeSearch
				&& this.message[historyElements[userId][i]].text.toLowerCase().indexOf((this.historySearch+'').toLowerCase()) < 0
				&& !isTextInParams
				&& !isTextInAuthorName
				&& !isTextInFileNames
			)
