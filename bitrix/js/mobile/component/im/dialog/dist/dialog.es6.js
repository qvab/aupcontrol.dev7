...
(function (exports,pull_components_status,main_date,mobile_pull_client,im_model,im_controller,im_provider_pull,im_provider_rest,im_tools_localstorage,im_tools_timer,im_tools_logger,im_const,im_utils,im_component_dialog,im_component_quotepanel,ui_vue_vuex,ui_vue) {
	...
	class MobileRestAnswerHandler extends im_provider_rest.BaseRestAnswerHandler
	{
		constructor(params)
		{
			super(params);
	...
	class Dialog
	{
		...
		constructor(params = {})
		{
			...
			window.addEventListener('orientationchange', () => {
				...
				if (
					this.store.state.application.device.type === im_const.DeviceType.mobile
					&& this.store.state.application.device.orientation === im_const.DeviceOrientation.horizontal
				)
				{
			...
			let serverVariables = im_tools_localstorage.LocalStorage.get(this.getSiteId(), 0, 'serverVariables', false);
			if (serverVariables)
			{
		...
		initStorage(data)
		{
			...
			return new ui_vue_vuex.VuexBuilder()
				.addModel(im_model.ApplicationModel.create().useDatabase(false).setVariables(applicationVariables))
				.addModel(im_model.MessagesModel.create())
				.addModel(im_model.DialoguesModel.create().setVariables({host: this.host}))
				.addModel(im_model.FilesModel.create().setVariables({host: this.host, default: {name: this.getLocalize('IM_MESSENGER_MESSAGE_FILE_DELETED')}}))
				.addModel(im_model.UsersModel.create().setVariables({host: this.host, default: {name: this.getLocalize('IM_MESSENGER_MESSAGE_USER_ANONYM')}}))
				.setDatabaseConfig({
		...
		initMobileEnvironment(result)
		{
			...
			BX.addCustomEvent("onKeyboardDidShow", () => {
				if (im_utils.Utils.platform.isAndroid())
				{
					this.controller.emit('EventType.dialog.scrollToBottom', {duration: 300, cancelIfScrollChange: true});
				}
			});
			...
			this.fileUpdateProgress = im_utils.Utils.throttle((chatId, fileId, progress, size) => {
		...
		initPullClient()
		{
			...
			mobile_pull_client.PULL.subscribe(
				this.pullCommandHandler = new im_provider_pull.ImPullCommandHandler({
					store: this.store,
					controller: this.controller,
					options: {
						handlingDialog: {
		...
		requestData()
		{
			...
			let query = {
				...
				[im_const.RestMethodHandler.imChatGet]: [im_const.RestMethod.imChatGet, {dialog_id: this.controller.getDialogId()}],
			...
			{
				query[im_const.RestMethodHandler.imUserListGet] = [im_const.RestMethod.imUserListGet, {id: [this.controller.getUserId(), this.controller.getDialogId()]}];
			}
			...
			this.restClient.callBatch(query, (response) =>
			{
				...
				let constGet = response[im_const.RestMethodHandler.mobileBrowserConstGet];
				if (constGet.error())
				;
				else
				{
					this.executeRestAnswer(im_const.RestMethodHandler.mobileBrowserConstGet, constGet);
				}

				let userGet = response[im_const.RestMethodHandler.imUserGet];
				if (userGet && !userGet.error())
				{
					this.executeRestAnswer(im_const.RestMethodHandler.imUserGet, userGet);
				}

				let userListGet = response[im_const.RestMethodHandler.imUserListGet];
				if (userListGet && !userListGet.error())
				{
					this.executeRestAnswer(im_const.RestMethodHandler.imUserListGet, userListGet);
				}
				...
				this.requestDataSend = false;
			}, false, false, im_utils.Utils.getLogTrackingParams({name: 'mobile.im.dialog', dialog: this.controller.getDialogData()}));
	...
	/* region 02. Mobile environment methods */
		...
		leaveChat(confirm = false)
		{
			...
			this.restClient.callMethod(im_const.RestMethod.imChatLeave, {DIALOG_ID: dialogId}, null, null, im_utils.Utils.getLogTrackingParams({
				...
				action: (data) =>
				{
					...
					else
					{
						attachedFiles.forEach(file => {
							...
							let fileType = im_model.FilesModel.getType(file.name);
							...
							if (
								fileType === im_const.FileType.image
								|| fileType === im_const.FileType.video
							)
							{
	...
	/* region 04. Rest methods */
		...
		sendMessage(message)
		{
			...
			BXMobileApp.Events.postToComponent('chatbackground::task::add', [
				...
				[im_const.RestMethod.imMessageAdd, {
		...
		{
			let fileType = im_model.FilesModel.getType(message.file.name);
			let fileExtension = message.file.name.toString().toLowerCase().split('.').splice(-1)[0];
			let attachPreviewFile = fileType !== im_const.FileType.image && message.file.preview;
			let needConvert = fileType === im_const.FileType.image && message.file.type !== 'image/gif' || fileType === im_const.FileType.video;
		...
		fileCommit(params, message)
		{
			...
			this.restClient.callMethod(im_const.RestMethod.imDiskFileCommit, queryParams, null, null, im_utils.Utils.getLogTrackingParams({
				...
				this.executeRestAnswer(im_const.RestMethodHandler.imDiskFileCommit, response, message);
			}).catch(error => {
				this.executeRestAnswer(im_const.RestMethodHandler.imDiskFileCommit, error, message);
			});
		...
		requestDiskFolderId()
		{
			...
			this.restClient.callMethod(im_const.RestMethod.imDiskFolderGet, {chat_id: this.controller.getChatId()}).then(response => {
				this.executeRestAnswer(im_const.RestMethodHandler.imDiskFolderGet, response);
				this.flagRequestDiskFolderIdSended = false;
				this.processSendMessages();
			}).catch(error => {
				this.flagRequestDiskFolderIdSended = false;
				this.executeRestAnswer(im_const.RestMethodHandler.imDiskFolderGet, error);
			});
		...
		getDialogHistory(lastId, limit = this.controller.getRequestMessageLimit())
		{
			this.restClient.callMethod(im_const.RestMethod.imDialogMessagesGet, {
				...
				this.executeRestAnswer(im_const.RestMethodHandler.imDialogMessagesGet, result);
				this.controller.emit(im_const.EventType.dialog.requestHistoryResult, {count: result.data().messages.length});
			}).catch(result => {
				this.controller.emit(im_const.EventType.dialog.requestHistoryResult, {error: result.error().ex});
			});
		}
		...
		getDialogUnread(lastId, limit = this.controller.getRequestMessageLimit())
		{
			...
			if (!lastId)
			{
				this.controller.emit(im_const.EventType.dialog.requestUnreadResult, {error: {error: 'LAST_ID_EMPTY', error_description: 'LastId is empty.'}});
				promise.reject();
				return promise;
			}
			...
			this.controller.readMessage(lastId, true, true).then(() =>
			{
				...
				this.restClient.callBatch(query, (response) =>
				{
					if (!response)
					{
						this.controller.emit(im_const.EventType.dialog.requestUnreadResult, {error: {error: 'EMPTY_RESPONSE', error_description: 'Server returned an empty response.'}});
					...
					let dialogMessageUnread = response[im_const.RestMethodHandler.imDialogMessagesGetUnread];
					if (dialogMessageUnread.error())
					{
						this.controller.emit(im_const.EventType.dialog.requestUnreadResult, {error: dialogMessageUnread.error().ex});
					}
					else
					{
						this.executeRestAnswer(im_const.RestMethodHandler.imDialogMessagesGetUnread, dialogMessageUnread);
				...
				}, false, false, im_utils.Utils.getLogTrackingParams({name: im_const.RestMethodHandler.imDialogMessagesGetUnread, dialog: this.controller.getDialogData()}));
			});
			...
			(this.headerMenu = HeaderMenu.create())
				.setItems([
					HeaderMenuItem.create('profile')
						...
						.setIcon('user')
						.skip(im_utils.Utils.dialog.isChatId(this.controller.getDialogId()))
					,
		...
		quoteMessage(id)
		{
			this.store.dispatch('dialogues/update', {
				...
				{
					setTimeout(() => this.controller.emit(im_const.EventType.dialog.scrollToBottom, {duration: 300, cancelIfScrollChange: true}), 300);
				}
				else
				{
					this.setTextFocus();
		...
		deleteMessage(id)
		{
			...
			let messageText = im_utils.Utils.text.purify(message.text, message.params, files, this.localize);
	...
	/* region 05. Interaction and utils */
		...
		executeBackgroundTaskSuccess(action, data)
		{
			...
			{
				this.executeRestAnswer(im_const.RestMethodHandler.imMessageAdd, successObject, data.extra);
			}
		}
	...
	ui_vue.Vue.component('bx-messenger',
	{
		...
		computed:
		{
			...
			localize()
			{
				return Object.assign({},
					ui_vue.Vue.getFilteredPhrases('MOBILE_CHAT_', this.$root.$bitrixMessages),
					ui_vue.Vue.getFilteredPhrases('IM_UTILS_', this.$root.$bitrixMessages),
				);
			},
		...
		methods:
		{
			...
			{
				im_tools_logger.Logger.info(name, ...params);
			},
...
//# sourceMappingURL=dialog.es6.js.map
