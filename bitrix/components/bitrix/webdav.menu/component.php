...
if ($arParams["PERMISSION"] >= "U" && $ob->workflow == "bizproc")
{
	$cache_id = $arParams["IBLOCK_ID"];
	$cache_path = $cache_path_main."bizproc";;
	if ($arParams["CACHE_TIME"] > 0 && $cache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_path))
	{
		$arTemplateStates = $cache->GetVars();
	}
	else
	{
		$arTemplateStates = 
		...
		if ($arParams["CACHE_TIME"] > 0)
		{
			...
			$cache->EndDataCache($arTemplateStates);
		}
	}
