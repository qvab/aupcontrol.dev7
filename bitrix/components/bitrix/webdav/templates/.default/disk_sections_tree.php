				...
				while ($filename = readdir($handle))
				{
					...
					if (is_dir($path.$filename))
					{
						...
						$arSection[$filename] = array(
							"NAME" => $filename, 
							"HAS_DIR" => __get_folder_tree($path.$filename, true)); 
					}
				}
				closedir($handle);
			}
...
/***************** BASE ********************************************/
	...
	$arParams["ACTIVE"] = trim($_REQUEST["active"]); 
	$ob = new CWebDavFile($arParams, ""); 
	if (!$ob->CheckRights("MOVE", false, $path = ""))
		$popupWindow->ShowError(GetMessage("WD_ACCESS_DENIED")); 
	
	$arParams["ELEMENT_ID"] = (empty($_REQUEST["element_id"]) ? "WD_IBLOCK_SECTION_ID" : $_REQUEST["element_id"]); 
/********************************************************************
				/Input params
********************************************************************/
...
********************************************************************/
	if ($_REQUEST["return_array"] == "Y")
	{
		...
		$result = array(
			"dir_id" => md5($_REQUEST["active"]), 
			"folder" => $_REQUEST["active"], 
			"subdirs" => __WDGetSectionsTree($ob->real_path_full.$_REQUEST["active"], "")); 
...
{
	BX.ajax.post(
		"<?=$APPLICATION->GetCurPageParam("", array("active"));?>", 
