...
if ($arParams["ACTION"] == "CLONE")
{
	...
	$arFields[] = 
		array("id" => "NAME", "name" => '<span class="required starrequired">*</span>'.GetMessage("WD_NAME"), "type" => "text", "value" => $arResult["ELEMENT"]["NAME"]); 
		...
		if (!empty($arDocumentStates))	
		{
			...
			foreach ($arDocumentStates as $key => $arDocumentState)
			{
				...
				if (count($arEvents) > 0)
				{
					...
					{
						$sData .= '<option value="'. htmlspecialchars($e["NAME"]).'"'.
		...
		if (!empty($arResult["ELEMENT"]["~arDocumentStates"]))	
		{
			foreach ($arResult["ELEMENT"]["~arDocumentStates"] as $key => $arDocumentState)
			{
				...
				if (count($arEvents) > 0)
				{
					...
					{
						$sData .= 
						'<option value="'. htmlspecialchars($e["NAME"]).'"'.(($_REQUEST["bizproc_event_".$bizProcIndex] == $e["NAME"]) ? ' selected="selected"' : '').'>' .
								htmlspecialchars($e["TITLE"]) . '</option>'; 
					}
					$sData .= '</select>'; 
				...
				if (strlen($arDocumentState["ID"]) > 0)
				{
					...
					if (count($arTasks) > 0)
					{
						...
						foreach ($arTasks as $arTask)
						{
							...
							$sData .= '<a href="'.$url.'" title="'.str_replace(array("&amp;quot;", "&quot;"), "", htmlspecialchars($arTask["DESCRIPTION"])).'">'. $arTask["NAME"] .'</a><br />'; 
						}
