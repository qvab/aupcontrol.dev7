...
$arUsersCache = array(
	intVal($GLOBALS["USER"]->GetId()) => array(
		"LOGIN" => htmlspecialcharsEx($GLOBALS["USER"]->GetLogin()),
		"NAME" => htmlspecialcharsEx($GLOBALS["USER"]->GetFirstName()),
		"LAST_NAME" => htmlspecialcharsEx($GLOBALS["USER"]->GetLastName())));
...
$res["URL"] = array(
	...
	"~DELETE" => CComponentEngine::MakePathFromTemplate($arParams["~ELEMENT_EDIT_URL"], 
			array("ELEMENT_ID" => $res["ID"], "ACTION" => "DELETE")));
$res["URL"]["DELETE"] = WDAddPageParams($res["URL"]["DELETE"], array("edit" => "y", "sessid" => bitrix_sessid()));
$res["URL"]["~DELETE"] = WDAddPageParams($res["URL"]["~DELETE"], array("edit" => "y", "sessid" => bitrix_sessid()));
/************** Permission *****************************************/
$arResult["ELEMENT"] = $res;
$res = array(
	"UNLOCK" => "N", 
	"EDIT" => ($arParams["PERMISSION"] >= "W" ? "Y" : "N"), 
	"DELETE" => ($arParams["PERMISSION"] >= "W" ? "Y" : "N"), 
	"HISTORY" => "Y");
if ($arResult["ELEMENT"]["LOCK_STATUS"] == "yellow" || 
	($arResult["ELEMENT"]["LOCK_STATUS"] == "red" && CWorkflow::IsAdmin()))
{
	$res["UNLOCK"] = "Y";
}
...
{
	$res["EDIT"] = (($arResult["ELEMENT"]["WF_STATUS_ID"] > 1 && 
		$arResult["WF_STATUSES_PERMISSION"][$arResult["ELEMENT"]["WF_STATUS_ID"]] < 2) ? "N" : "Y");
}
...
if ($db_res)
{
	...
	if ($res = $db_res->GetNext())
	{
		do
		{
			...
			$res["WF_STATUS_TITLE"] = htmlspecialcharsEx($res["~WF_STATUS_TITLE"]);
			$res["SHOW"] = array(
				"RESTORE" => ($arParams["PERMISSION"] > "W" ? "Y" : "N"), 
				"DELETE" => ($arParams["PERMISSION"] > "W" ? "Y" : "N"));
			...
			$res["URL"]["RESTORE"] = WDAddPageParams($res["URL"]["RESTORE"], 
				array("history_id" => $res["ID"], "action" => "restore", "edit" => "y", "sessid" => bitrix_sessid()));
			$res["URL"]["~RESTORE"] = WDAddPageParams($res["URL"]["~RESTORE"], 
				array("history_id" => $res["ID"], "action" => "restore", "edit" => "y", "sessid" => bitrix_sessid()));
				
			$res["URL"]["DELETE"] = WDAddPageParams($res["URL"]["RESTORE"], 
				array("history_id" => $res["ID"], "action" => "delete", "edit" => "y", "sessid" => bitrix_sessid()));
			$res["URL"]["~DELETE"] = WDAddPageParams($res["URL"]["~RESTORE"], 
				array("history_id" => $res["ID"], "action" => "delete", "edit" => "y", "sessid" => bitrix_sessid()));
			...
			$aCols = array(
				...
				"MODIFIED_BY" => (empty($arUsersCache[$res["MODIFIED_BY"]]) ? 
					...
					'[<a href="'.$arUsersCache[$res["MODIFIED_BY"]]["URL"].'">'.$res["MODIFIED_BY"].'</a>] ('.
						$arUsersCache[$res["MODIFIED_BY"]]["LOGIN"].") ".$arUsersCache[$res["MODIFIED_BY"]]["NAME"]." ".
						$arUsersCache[$res["MODIFIED_BY"]]["LAST_NAME"])
				);
...
if ((!empty($_POST["HISTORY_ID"]) || !empty($_GET["history_id"])) && check_bitrix_sessid())
{
	...
	foreach($arID as $ID)
	{
		...
		if($dr = $d->Fetch())
		{
			...
			{
				if ($arParams["PERMISSION"] < "W" && !($dr["WF_STATUS_ID"] > 1 && $arResult["WF_STATUSES_PERMISSION"][$dr["WF_STATUS_ID"]] >= 2))
	...
	$url = CComponentEngine::MakePathFromTemplate($arParams["~ELEMENT_HISTORY_URL"], 
			array("ELEMENT_ID" => $arParams["ELEMENT_ID"]));
	LocalRedirect($url);
}
/********************************************************************
