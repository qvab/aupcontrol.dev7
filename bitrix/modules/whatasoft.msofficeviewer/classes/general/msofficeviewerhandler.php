...
class CMSOfficeViewerHandler extends DocumentHandler implements IViewer {
	const ERROR_METHOD_IS_NOT_SUPPORTED = 'DISK_MSOV_HANDLER_22001';
	const ERROR_COULD_NOT_FIND_EXT_LINK = 'DISK_MSOV_HANDLER_22002';
	const ERROR_FORMAT_IS_NOT_SUPPORTED = 'DISK_MSOV_HANDLER_22003';
	const VIEWER_URL = 'https://view.officeapps.live.com/op/embed.aspx';
	...
	public function createBlankFile(FileData $fileData) {
		$this->errorCollection->add(array(
			new Error(Loc::getMessage('DISK_MSOFFICE_VIEWER_HANDLER_ERROR_METHOD_IS_NOT_SUPPORTED'), self::ERROR_METHOD_IS_NOT_SUPPORTED)
		));
	...
	public function getDataForViewFile(FileData $fileData) {
		$fileExtension = strtolower(
			pathinfo($fileData->getName(), PATHINFO_EXTENSION)
		);

		if (!in_array($fileExtension, $this->allowedFileExtensions)) {
			$this->errorCollection->add(array(
				new Error(Loc::getMessage('DISK_MSOFFICE_VIEWER_HANDLER_ERROR_FORMAT_IS_NOT_SUPPORTED'), self::ERROR_FORMAT_IS_NOT_SUPPORTED)
			));
		...
		if (!$extLinkModel) {
			$this->errorCollection->add(array(new Error(Loc::getMessage('DISK_MSOFFICE_VIEWER_HANDLER_ERROR_COULD_NOT_FIND_EXT_LINK'), self::ERROR_COULD_NOT_FIND_EXT_LINK)));
		...
		$accessibilityMode = Option::get('whatasoft.msofficeviewer', 'msov_use_accessibility_mode', 'Y');

		return array(
			'id' => $extLinkModel->getHash(),
			'viewUrl' => self::VIEWER_URL . '?' . http_build_query(array(
				'src' => $extLink,
				'wdAccPdf' => (int)($accessibilityMode == 'Y'),
			)),
			'neededDelete' => false,
	...
	public function checkViewFile(FileData $fileData) {
		...
		if (!$extLinkModel) {
			$this->errorCollection->add(array(
				new Error(Loc::getMessage('DISK_MSOFFICE_VIEWER_HANDLER_ERROR_COULD_NOT_FIND_EXT_LINK'), self::ERROR_COULD_NOT_FIND_EXT_LINK)
			));
