...
if($sellerInfo['ADDRESS'] || $customerInfo['ADDRESS'])
{
	...
	foreach (array($sellerInfo['ADDRESS'], $customerInfo['ADDRESS']) as $text)
	{
		if ($text)
		{
			...
			$text = Loc::getMessage('SBLP_Q_UA_TEXT_ADDRESS', null, $lng).': '.$text;
...
{
	$text = Loc::getMessage('SBLP_Q_UA_TEXT_CONTACT_PERSON', null, $lng).': '.$customerInfo['PAYER_NAME'];
	$cols[1][$colRows[1]++] = $text;
}
if($sellerInfo['PHONE'] || $customerInfo['PHONE'])
{
	...
	foreach (array($sellerInfo['PHONE'], $customerInfo['PHONE']) as $text)
	{
		if ($text)
		{
			$text = Loc::getMessage('SBLP_Q_UA_TEXT_PHONE', null, $lng).': '.$text;
			$cols[$i][$colRows[$i]++] = $text;
		}
		$i++;
	}
...
if($customerInfo['FAX'])
{
	$text = Loc::getMessage('SBLP_Q_UA_TEXT_FAX', null, $lng).': '.$customerInfo['FAX'];
	$cols[1][$colRows[1]++] = $text;
}
...
{
	$i = 0;
	foreach (array($sellerInfo['EDRPOU'], $customerInfo['EDRPOU']) as $text)
	{
		if ($text)
		{
			$text = Loc::getMessage('SBLP_Q_UA_TEXT_EDRPOU', null, $lng).': '.$text;
			$cols[$i][$colRows[$i]++] = $text;
		}
		$i++;
	}
...
if($sellerInfo['RS'])
{
	$text = Loc::getMessage('SBLP_Q_UA_TEXT_ACCOUNT', null, $lng).': '.$sellerInfo['RS'];
	$cols[0][$colRows[0]++] = $text;
}
if($sellerInfo['BANK'])
{
	...
	$text = Loc::getMessage('SBLP_Q_UA_TEXT_BANK', null, $lng).': '.
		strval($sellerInfo['BANK']).($text ? ', '.$text : '');
	$cols[0][$colRows[0]++] = $text;
}
if($sellerInfo['MFO'])
{
	$text = Loc::getMessage('SBLP_Q_UA_TEXT_MFO', null, $lng).': '.$sellerInfo['MFO'];
	$cols[0][$colRows[0]++] = $text;
}
unset($text);
