...
class CCrmSaleHelper
{
	...
	public static function isShopAccess($role = "")
	{
		...
		if ($isCrmAccess && !$isDbAccess)
		{
			$shopRole = self::getShopRole($USER);
			if ($shopRole)
			{
				self::addToDbAccess($USER, $shopRole);
				self::addToCacheAccess($userId, $role, true);
				return true;
			}
			else
	...
	private static function getShopRole(CAllUser $user)
	{
		$userId = $user->getID();
		if ($user->isAdmin() || (Loader::includeModule("bitrix24") && CBitrix24::IsPortalAdmin($userId)))
		{
	...
	private static function addToDbAccess(CAllUser $user, $shopRole)
	{
