		...
		foreach ($arID as $subID)
		{
			...
			case ActionType::DEACTIVATE:
				if (CIBlockElementRights::UserHasRightTo($intSubIBlockID, $subID, "element_edit"))
				{
					...
					$arFields = array("ACTIVE"=>($_REQUEST['action'] == ActionType::ACTIVATE ? "Y": "N"));
...
if (!empty($arRows))
{
	...
	foreach ($arRows as $itemId => $row)
	{
		...
		$clearCounter = array(
			...
			"ACTION" => "if (confirm('".GetMessageJS("IBSEL_A_CLEAR_COUNTER_CONFIRM")."')) ".$lAdmin->ActionDoGroup($row->arRes['orig']['ID'], ActionType::CLEAR_COUNTER, $sThisSectionUrl),
			"ONCLICK" => ""
		);
		...
		elseif ($boolSubBizproc)
		{
			...
			if ($row->arRes['lockStatus'] == "red")
			{
				if (CBPDocument::IsAdmin())
				{
					$arActions[] = Array(
						...
						"ACTION" => "if (confirm('".GetMessageJS("IBEL_A_UNLOCK_CONFIRM")."')) ".$lAdmin->ActionDoGroup($itemId, ActionType::ELEMENT_UNLOCK, $sThisSectionUrl),
					);
				}
			}
			elseif ($bWritePermission)
			{
				...
				$arActions[] = array(
					...
					"TEXT" => GetMessage('MAIN_DELETE'),
					"TITLE" => GetMessage("IBLOCK_DELETE_ALT"),
					"ACTION" => "if (confirm('".GetMessageJS('IBLOCK_CONFIRM_DEL_MESSAGE')."')) ".$lAdmin->ActionDoGroup($itemId, ActionType::DELETE, $sThisSectionUrl),
				);
			}
		}
		else
	...
	foreach ($arElementOps as $id => $arOps)
	{
		...
		{
			$arGroupActions[ActionType::DELETE] = GetMessage("MAIN_ADMIN_LIST_DELETE");
			break;
		}
	}
	foreach ($arElementOps as $id => $arOps)
	{
		...
		{
			$arGroupActions[ActionType::ACTIVATE] = GetMessage("MAIN_ADMIN_LIST_ACTIVATE");
			$arGroupActions[ActionType::DEACTIVATE] = GetMessage("MAIN_ADMIN_LIST_DEACTIVATE");
			$arGroupActions[ActionType::CLEAR_COUNTER] = strtolower(GetMessage('IBSEL_A_CLEAR_COUNTER'));
			break;
		}
	}
	...
	if ($bWorkFlow)
	{
		$arGroupActions[ActionType::ELEMENT_UNLOCK] = GetMessage("IBEL_A_UNLOCK_ACTION");
		$arGroupActions[ActionType::ELEMENT_LOCK] = GetMessage("IBEL_A_LOCK_ACTION");

		$statuses = '<div id="wf_status_id" style="display:none">'.SelectBox("wf_status_id", CWorkflowStatus::GetDropDownList("N", "desc")).'</div>';
		$arGroupActions[ActionType::ELEMENT_WORKFLOW_STATUS] = GetMessage("IBEL_A_WF_STATUS_CHANGE");
