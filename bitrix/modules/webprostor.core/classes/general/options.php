...
Class CWebprostorCoreOptions
{
	...
	public function ShowHTML()
	{
		...
		if(is_array($this->arOptions))
		{
			foreach($this->arOptions as $option => $arOptParams)
			{
				...
				switch($arOptParams['TYPE'])
				{
					...
					default:
						...
						$input = '<input type="'.($arOptParams['TYPE'] == 'INT' ? 'number' : ($arOptParams['TYPE'] == 'PASSWORD' ? 'password' :'text')).'" size="'.$arOptParams['SIZE'].'" maxlength="'.$arOptParams['MAXLENGTH'].'" value="'.htmlspecialchars($val).'" name="'.htmlspecialchars($option).'" />';
						if($arOptParams['REFRESH'] == 'Y')
							$input .= '<input type="submit" name="refresh" value="OK" />';
						break;
				}
			...
			echo 	'<input type="submit" name="restoreDefault" value="'.GetMessage("WEBPROSTOR_CORE_OPTIONS_RESET").'"'.(($moduleAccessLevel < "W")?" disabled":"").' />';
