...
require_once $_SERVER["DOCUMENT_ROOT"]."/local/classes_miwr/__utility.php";
class __core extends __Utility
{
  ...
  protected function __dbConnect()
  {
    $bd_host = "10.0.10.30";
    $bd_user = "onlyoffice";
    $bd_password = "onlyoffice";
    $bd_base = "onlyoffice_helper";
    $this->resourceConnect = mysqli_connect($bd_host, $bd_user, $bd_password, $bd_base);
    mysqli_set_charset($this->resourceConnect, "UTF-8");
    mysqli_query($this->resourceConnect,"SET NAMES 'utf8';");
    mysqli_query($this->resourceConnect,"SET CHARACTER SET 'utf8';");
    mysqli_query($this->resourceConnect,"SET SESSION collation_connection = 'utf8_general_ci';");
  }
  ...
  protected function __dbGet($arParam)
  {
    ...
    if (empty($this->errors)) {
      ...
      $rResponse = $this->__dbQuery($sSql) or die($this->__dbError());
      return !empty($this->__dbError()) ? $this->__dbError() : $rResponse;
    }
